...
def prepare_ticket_context(ticket):
    ...
    return {
        'id': ticket.id,
        'ticket_number': ticket.ticket_number if ticket.ticket_number else str(ticket.id),
        'title': ticket.title,
        'description': ticket.description if ticket.description else 'No description provided',
...
def send_ticket_created_notification(ticket):
    ...
    subject = f"New Ticket Created: #{context['ticket']['ticket_number']} - {ticket.title}"
...
def send_comment_notification(comment, ticket):
    ...
    if comment.author == ticket.created_by:
        ...
        if ticket.assigned_to and ticket.assigned_to.email and comment.author != ticket.assigned_to:
...
def send_ticket_updated_notification(ticket, changed_fields, updated_by):
    ...
    if updated_by.is_staff:
        ...
        if ticket.assigned_to and ticket.assigned_to.email and updated_by != ticket.assigned_to:
            recipients.append(ticket.assigned_to.email)
