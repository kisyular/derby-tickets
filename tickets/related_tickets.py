...
class RelatedTicketsFinder:
    ...
    def _find_by_category(self):
        ...
        ).exclude(id=self.ticket.id).values_list('id', flat=True)[:10]
        
        return [(tid, 0.7, f"Same category: {self.ticket.category.name}") for tid in related]
    ...
    def _find_by_references(self):
        ...
        ).exclude(id=self.ticket.id).values_list('id', flat=True)
    ...
    def _find_by_keywords(self):
        ...
        for keyword in keywords[:5]:  # Top 5 keywords
            ...
            ).exclude(id=self.ticket.id).values_list('id', flat=True)[:5]
    ...
    def _find_by_creator_pattern(self):
        ...
        creator_tickets = Ticket.objects.filter(
    ...
    def get_similar_tickets(target_ticket, threshold=0.3):
        ...
        for ticket in Ticket.objects.exclude(id=target_ticket.id):
            ...
            if similarity > threshold:
                similar_tickets.append((ticket.id, similarity, f"Text similarity: {similarity:.2f}"))
        
        return similar_tickets


# Usage example:
def get_related_tickets_for_display(ticket):
