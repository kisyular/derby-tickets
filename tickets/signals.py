        ...
        if created:
            ...
            if hasattr(instance, '_old_priority') and instance._old_priority != instance.priority:
                ...
                changed_fields['priority'] = {
                    'old': priority_choices.get(instance._old_priority, instance._old_priority),
            ...
            if hasattr(instance, '_old_status') and instance._old_status != instance.status:
                ...
                changed_fields['status'] = {
                    'old': status_choices.get(instance._old_status, instance._old_status),
            ...
            if old_assigned != instance.assigned_to:
                if instance.assigned_to:
                    # Ticket was assigned
                    logger.info(f"Ticket {instance.id} assigned to {instance.assigned_to}")
                    send_ticket_assigned_notification(instance)
                
                # Track assignment change for update notification
                changed_fields['assigned_to'] = {
                    'old': old_assigned.get_full_name() if old_assigned else 'Unassigned',
                    'new': instance.assigned_to.get_full_name() if instance.assigned_to else 'Unassigned'
                }
            
            # Send update notification if there were significant changes
            if changed_fields:
                logger.info(f"Ticket {instance.id} updated with changes: {list(changed_fields.keys())}")
                # Get the user who made the change (if available from request)
                updated_by = getattr(instance, '_updated_by', None)
                if updated_by:
                    send_ticket_updated_notification(instance, changed_fields, updated_by)
                else:
...
def comment_saved(sender, instance, created, **kwargs):
    ...
    if created:
        try:
            logger.info(f"New comment added to ticket {instance.ticket.id} by {instance.author}")
